...
@server.agent()
async def policy_agent(input: list[Message]) -> AsyncGenerator[RunYield, RunYieldResume]:    # Runs the agent with the ability to await inputs
    ...
    task1 = Task(
         description=input[0].parts[0].content,    # This is now a dynamic prompt that is coming from a user interacting with the server
...
if __name__ == "__main__":
    server.run(port=8001)  # Here we are running on a local port, but can specify a service to run our server from
